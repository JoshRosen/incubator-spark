 ...
 */
private[spark] abstract class Task[T](val stageId: StageId, var partitionId: Int)
  extends Serializable {

  final def run(attemptId: Long): T = {
    context = new TaskContext(stageId.underlying, partitionId, attemptId, runningLocally = false)
    if (_killed) {
      kill()
    }
    runTask(context)
